...
static void Discord_RegisterW(const wchar_t* applicationId, const wchar_t* command)
{
	...
	if (command && command[0]) {
		StringCbPrintfW(openCommand, sizeof(openCommand), E(L"%s"), command);
	}
	else {
		// StringCbCopyW(openCommand, sizeof(openCommand), exeFilePath);
		StringCbPrintfW(openCommand, sizeof(openCommand), E(L"%s"), exeFilePath);
	}

	wchar_t protocolName[64];
	StringCbPrintfW(protocolName, sizeof(protocolName), E(L"discord-%s"), applicationId);
	wchar_t protocolDescription[128];
	StringCbPrintfW(
		protocolDescription, sizeof(protocolDescription), E(L"URL:Run game %s protocol"), applicationId);
	wchar_t urlProtocol = 0;

	wchar_t keyName[256];
	StringCbPrintfW(keyName, sizeof(keyName), E(L"Software\\Classes\\%s"), protocolName);
	HKEY key;
	auto status =
		RegCreateKeyExW(HKEY_CURRENT_USER, keyName, 0, nullptr, 0, KEY_WRITE, nullptr, &key, nullptr);
	if (status != ERROR_SUCCESS) {
		fprintf(stderr, E("Error creating key\n"));
		return;
	}
	DWORD len;
	LSTATUS result;
	...
	if (FAILED(result)) {
		fprintf(stderr, E("Error writing description\n"));
	}

	len = (DWORD)lstrlenW(protocolDescription) + 1;
	result = RegSetKeyValueW(key, nullptr, E(L"URL Protocol"), REG_SZ, &urlProtocol, sizeof(wchar_t));
	if (FAILED(result)) {
		fprintf(stderr, E("Error writing description\n"));
	}
	...
	if (FAILED(result)) {
		fprintf(stderr, E("Error writing icon\n"));
	}

	len = (DWORD)lstrlenW(openCommand) + 1;
	result = RegSetKeyValueW(
		key, E(L"shell\\open\\command"), nullptr, REG_SZ, openCommand, len * sizeof(wchar_t));
	if (FAILED(result)) {
		fprintf(stderr, E("Error writing command\n"));
	}
	RegCloseKey(key);
}
	...
	const char* steamId)
{
	...
	HKEY key;
	auto status = RegOpenKeyExW(HKEY_CURRENT_USER, E(L"Software\\Valve\\Steam"), 0, KEY_READ, &key);
	if (status != ERROR_SUCCESS) {
		fprintf(stderr, E("Error opening Steam key\n"));
		return;
	}

	wchar_t steamPath[MAX_PATH];
	DWORD pathBytes = sizeof(steamPath);
	status = RegQueryValueExW(key, E(L"SteamExe"), nullptr, nullptr, (BYTE*)steamPath, &pathBytes);
	RegCloseKey(key);
	if (status != ERROR_SUCCESS || pathBytes < 1) {
		fprintf(stderr, E("Error reading SteamExe key\n"));
		return;
	}
	...
	StringCbPrintfW(command, sizeof(command), E(L"\"%s\" steam://rungameid/%s"), steamPath, wSteamId);
